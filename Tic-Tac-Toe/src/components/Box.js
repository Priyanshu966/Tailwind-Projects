...
const Box = () => {
  const {isTurn, handleTurn,setResetFalse,isReset} = useGlobalContext();
  ...
  const handleBox = (e) => {
    ...
    if (isTurn === "circle" && cross.classList.contains("hidden")) {
      e.currentTarget.firstElementChild.classList.remove("hidden");
      handleTurn();
    } else if(isTurn === "cross"  && circle.classList.contains("hidden")) {
